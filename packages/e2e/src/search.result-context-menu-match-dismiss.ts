...
export const test: Test = async ({ Command, expect, FileSystem, Locator, Search, SideBar, Workspace }) => {
  ...
  const dismissedMatch = Locator('.TreeItem[aria-label="abc"]')
  const remainingMatch = Locator('.TreeItem[aria-label="abx"]')
  await expect(message).toHaveText('1 result in 1 file')
  await expect(dismissedMatch).toBeHidden()
  await expect(remainingMatch).toBeVisible()
}
