...
test('openSearchEditor - opens a search editor in the main area', async () => {
  ...
  expect(mockRpc.invocations[0][1]).toMatch(/^search-editor:\/\/42-\d+\/Search$/)
...
test('openSearchEditor - opens a new editor on every invocation', async () => {
  ...
  expect(firstUri).toMatch(/^search-editor:\/\/7-\d+\/Search$/)
  expect(secondUri).toMatch(/^search-editor:\/\/7-\d+\/Search$/)
})
